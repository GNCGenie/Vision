...
int main()
{
    const char* video_file = "/dev/video0";
    cv::Size board_pattern(9, 6);
    float board_cellsize = 0.02475f;
    ...
    while (true)
    {
        ...
        if (select_images)
        {
            ...
            cv::Mat downscaled = image.clone();
            cv::resize(downscaled, downscaled, cv::Size(640, 480));
            cv::imshow("Camera Calibration", downscaled);
